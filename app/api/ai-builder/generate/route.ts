...
import { buildGenerateSystemPrompt, parseGeneratedSite } from '@/lib/aiBuilderSchema'
import { generateMockSite } from '@/lib/aiBuilderMock'
...
export async function POST(req: NextRequest) {
  ...
  if (await isRateLimited(`ai-builder-generate:${ip}`, 5, 3600)) {
    return NextResponse.json(
      { error: 'Too many website generations. Please try again in a bit.' },
      { status: 429 }
    )
  }
  ...
  if (process.env.AI_BUILDER_PROVIDER === 'mock') {
    return NextResponse.json({ site: generateMockSite(description) })
  }
  ...
  const result = await callOpenRouterJson({
    ...
    timeoutMs: 60000,
    usageFeature: 'ai-builder-generate',
  })
  ...
  if (!site) {
    return NextResponse.json({ error: "That didn't quite work — please try rephrasing your business description." }, { status: 502 })
  }
